 ...
 */
public class BaseDatasetWaiso implements Serializable, Dataset {
	...
	public static BaseDatasetWaiso load(String file) {
		Object o = null;
		...
		return (BaseDatasetWaiso) o;
	}

	public static void save(String file, BaseDatasetWaiso o) {
		try {
			File f = new File(file);
	...
	public ContentProduto pickContentItem(String name) {
		ContentProduto contentItem = null;

		for (Map.Entry<Integer, Item> entry : allItems.entrySet()) {
			Item anItem = entry.getValue();
			if (name.equals(anItem.getName())) {
				contentItem = new ContentProduto(entry.getValue());
				break;
			}
		}
		return contentItem;
	...
	public void save(String file) {
		BaseDatasetWaiso.save(file, this);
